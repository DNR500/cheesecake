...
(function () {
    ...
    function configurePackageDir(packages, currentPackages, dir) {
        ...
        for (i = 0; (pkgObj = currentPackages[i]); i++) {
            ...
            if (dir &&
                (!location || (location.indexOf("/") !== 0 && location.indexOf(":") === -1))) {
    ...
    main = function (name, deps, callback, config, contextName, relModuleName) {
        ...
        contextName =
            contextName ? contextName : (config && config.context ? config.context : s.ctxName);
    ...
    req.modify = function (target, name, deps, callback, contextName) {
        ...
        if (typeof target === "string") {
            ...
            if (context.specified[target]) {
                ...
                (list.__deferMods || (list.__deferMods = [])).push([
                    name,
                    deps,
                    callback,
                    contextName
    ...
    req.exec = function (module, traced, waiting, context) {
            ...
            for (j = 0; (dep = deps[j]); j++) {
                ...
                if (depName === "require") {
                    ...
                    depModule = depName in defined ? defined[depName] :
                        (traced[depName] ? undefined :
                            req.exec(waiting[waiting[depName]], traced, waiting, context));
                }
        ...
        if (cb && req.isFunction(cb)) {
            ...
            if (name) {
                ...
                if (usingExports && ret === undefined &&
                    (!cjsModule || !("exports" in cjsModule))) {
                    ret = defined[name];
                } else {
    ...
    require.plugin({
        ...
        load: function (name, contextName) {
            ...
            if (match[5]) {
                ...
                if (context.nlsRootLoaded[masterName] && bundle) {
                    ...
                    (context.nlsToLoad[masterName] ||
                        (context.nlsToLoad[masterName] = [])).push(locale);
        ...
        orderDeps: function (context) {
            ...
            for (i = 0; (msWaiting = nlsWaiting[i]); i++) {
                ...
                //together.
                context.defined[master] =
                    context.defined[modulePrefix + "/" + defLoc + "/" + moduleSuffix];

                //Handle any best fit locale definitions.
                if (bestFit) {
                    for (loc in bestFit) {
                        if (!(loc in empty)) {
                            context.defined[modulePrefix + "/" + loc + "/" + moduleSuffix] =
                                context.defined[modulePrefix + "/" + bestFit[loc] + "/" +
                                    moduleSuffix];
                        }
                    }
                }
